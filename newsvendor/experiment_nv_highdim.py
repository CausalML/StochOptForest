...
direct = ''
date = ''
output = direct + date + "nv_highdim.txt"
...
for N in N_list:
    ...
    for p in p_list:
        ...
        results_fit = Parallel(n_jobs=n_jobs, verbose = 3)(delayed(compare_adaptive_nonadaptive_one_run)(X_list[run], Y_list[run], X_list[run], Y_list[run], 
            ...
            n_proposals = n_proposals, balancedness_tol = balancedness_tol, bootstrap = bootstrap, seed = seed) for run in range(runs))
        time2 = time.time()
        with open(output, 'a') as f:
        ...
        results_eval = Parallel(n_jobs=n_jobs, verbose = 3)(delayed(evaluate_one_run)(results_fit[run], X_list[run], Y_list[run], X_list[run], Y_list[run], 
            ...
            h_list =h_list, b_list = b_list, C = C, verbose = verbose, seed = seed) for run in range(runs))
        time2 = time.time()
        ...
        fi = evaluate_feature_importance(results_fit, p)
        fi_all[str(N)][str(p)] = fi

        pickle.dump(risk_all, open(direct + date +  "risk_highdim.pkl", "wb"))
        pickle.dump(feature_split_all, open(direct + date +  "feature_split_highdim.pkl", "wb"))
        pickle.dump(fi_all, open(direct + date +  "feature_importance_highdim.pkl", "wb"))
        pickle.dump(results_eval_all, open(direct + date +  "results_eval_highdim.pkl", "wb"))